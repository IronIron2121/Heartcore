...
- `sourcemap.json` does **not** contain workspace children (parts, models, zones, etc.) — do not use it to understand the Workspace hierarchy
- It only reflects instances backed by source files (scripts, modules) — useful for discovering script paths, not world geometry
- To understand workspace structure (zones, stage parts, spawn locations, etc.), ask the user directly
